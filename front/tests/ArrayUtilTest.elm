        ...
        [ describe "Util"
            [ describe "all"
                [ fuzz (array <| constant True) "of identity with an array of True values is True" <|
                    \subject ->
                        Expect.equal True (Util.all identity subject)
                , fuzz (array <| intRange 1 Random.maxInt) "of isPositive with array of positive integers is True" <|
                    \subject ->
                        Expect.equal True (Util.all (\n -> n > 0) subject)
                ]
            , describe "any"
                [ fuzz (array <| constant True) "of identity with an non empty array of True values is True" <|
                    \subject ->
                        Expect.equal True <| (Array.length subject == 0) || (Util.any identity <| subject)
                , fuzz (array <| intRange 1 Random.maxInt) "of isPositive with array of positive integers is True" <|
                    \subject ->
                        Expect.equal True <| (Array.length subject == 0) || Util.any (\n -> n > 0) subject
                ]
            ]
        ]
